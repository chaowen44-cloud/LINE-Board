...
import { Trash2, Edit2, Check, Plus, Eye, Megaphone, Target, Percent, TrendingUp, AlertCircle, GripVertical } from "lucide-react";
...
export default function LapAdsSection({ items, onUpdateItems, onPreviewImage }: LapAdsSectionProps) {
  ...
  const handleReorder = (dragIndex: number, hoverIndex: number) => {
    ...
    const targetItem = nextItems[dragIndex];
    nextItems.splice(dragIndex, 1);
    nextItems.splice(hoverIndex, 0, targetItem);
                ...
                return (
                  <tr 
                    ...
                    className={`transition-all duration-150 border-b ${
                      ...
                      isOver ? "bg-blue-50 border-t-2 border-b-2 border-t-[#00b8db] border-b-[#00b8db]" : "border-slate-100"
                    }`}
                  >
                    {/* Drag Handle */}
                    <td className="px-2 py-3 text-center align-middle">
                      <div 
                        ...
                        title={isEditing ? "Save editing to drag row" : "Drag to reorder lap ad campaign"}
                      >
