...
import { Trash2, Edit2, Check, Plus, Calendar, AlertCircle, GripVertical } from "lucide-react";
...
export default function TimelineSection({
  ...
  const handleReorder = (dragIndex: number, hoverIndex: number) => {
    // Determine the elements in their filtered position
    const targetItem = filteredItems[dragIndex];
    const hoverItem = filteredItems[hoverIndex];

    const masterDragIdx = items.findIndex(item => item.id === targetItem.id);
    const masterHoverIdx = items.findIndex(item => item.id === hoverItem.id);

    if (masterDragIdx !== -1 && masterHoverIdx !== -1) {
      const nextItems = [...items];
      nextItems.splice(masterDragIdx, 1);
      nextItems.splice(masterHoverIdx, 0, targetItem);
                ...
                return (
                  <tr 
                    ...
                    className={`transition-all duration-150 border-b ${
                      isDragging ? "opacity-30 bg-slate-100 border-2 border-dashed border-slate-300 scale-[0.98]" : "hover:bg-slate-50/50"
                    } ${
                      isOver ? "bg-blue-50 border-t-2 border-b-2 border-t-blue-500 border-b-blue-500" : "border-slate-100"
                    }`}
                  >
                    {/* Drag Handle */}
                    <td className="px-2 py-3 text-center align-middle">
                      <div 
                        className={`text-slate-400 hover:text-slate-600 inline-flex items-center justify-center p-1 rounded-sm hover:bg-slate-100 cursor-grab active:cursor-grabbing transition-colors ${
                          isEditing ? "opacity-20 cursor-not-allowed pointer-events-none" : ""
                        }`}
                        title={isEditing ? "Save editing to drag row" : "Drag to reorder project"}
                      >
                    ...
                    <td className="px-5 py-3">
                      {isEditing ? (
                        ...
                        <p className={`text-sm max-w-[320px] transition-colors break-words ${item.completed ? "text-slate-400 line-through" : "text-slate-600"}`} title={item.note}>
